...
`zbl` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/zbl/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/zbl/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee zbl` to
...
python ./.trellis/scripts/task.py archive 00-join-zbl
```
